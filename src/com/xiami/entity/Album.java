...
public class Album implements SqlEntity{
	...
	public boolean save2DB(Connection conn) {
		try {
			...
			// albums
			stmt = conn.prepareStatement("replace into albums values(?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			for(long genre_id : genres) {
				stmt = conn.prepareStatement("replace into album_genre values(?,?)");
				stmt.setLong(1, album_id);
			...
			for(long tag_id : tags) {
				stmt = conn.prepareStatement("replace into album_tag values(?,?)");
				stmt.setLong(1, album_id);
